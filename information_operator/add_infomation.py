...
class Add_info(object):
    ...
    def add_obj(obj_table):
        ...
        if obj_table == 'school_manage':
            ...
            print('\033[31;1m请先输入教师所属学校的ID，再输入教师的其他信息！\033[0m')
            school_id = input('请输入学校ID：')
            search_infomation.Search_info.search_id_in_table(school_id, obj_table)
        ...
        if not result:
            print('\033[31;1m此%s已存在,请检查后重新输入。\033[0m' % Add_info.menu_dict[obj_table])
            print()
