...
class Add_info():
    ...
    def add_obj(obj_table):
        ...
        if result == 'Fail':
            ...
            print()
            return manage_page.manage_info(obj_table)
        elif result == 'Success':
    ...
    def add_course_obj(obj_table):
        ...
        school_obj = search_infomation.Search_info.search_id(school_id, 'school_manage')
        if school_obj == 'Fail':
        ...
        search_course_id_result = search_infomation.Search_info.search_id(course_id, obj_table)
    ...
    def add_classes(obj_table):
        ...
        teacher_obj = search_infomation.Search_info.search_id(teacher_id, 'teacher_manage')
        if teacher_obj == 'Fail':
            print('您输入的教师ID有误，请查询后再操作！')
            return main_master.manage_view
        course_id = input('请输入所教课程的ID：')
        course_obj = search_infomation.Search_info.search_id(course_id, 'course_manage')
        if course_obj == 'Fail':
        ...
        search_classes_id = search_infomation.Search_info.search(classes_id, 'classes_manage')
    ...
    def add_teacher_obj(obj_table):
        ...
        search_teacher_id_result = search_infomation.Search_info.search_id(teacher_id, obj_table)
    ...
    def add_student_obj(obj_table):
        ...
        classes_obj = search_infomation.Search_info.search_id(classes_id, 'classes_manage')
        if classes_obj == 'Fail':
            print()
    ...
    def add_buy_course(self, db_table, buy_obj):
        print('************目前所开课程有************')
        search_infomation.Search_info.search_all_obj(buy_obj)
        user_buy_course_id = input('\033[31;1m请输入您需要购买的课程ID：\033[0m')
        # 获取课程对象
        course_obj = search_infomation.Search_info.search_id(user_buy_course_id, buy_obj)
        if course_obj:
            obj_val = [self, course_obj, datetime.date.fromtimestamp(time.time())]
            print('add_infomation.py line 168')
            print(obj_val)
            db_operator.operator_db.add_obj_to_db(obj_val, db_table)
        else:
