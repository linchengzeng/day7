...
class Editor_info():
    ...
    def edit_obj_info(obj_table):
        if obj_table == 'school_manage':
            ...
            print('edit_obj_infomation.py line 22')
        elif obj_table == 'classes_manage':
            Editor_info.editor_classes_info(obj_table)
    ...
    def editor_school_obj(obj_table):
        ...
        if obj_info == 'Fail':
            ...
            if update_attr in style.school_attr:
                ...
                try:
                    # 执行更新学校信息操作
                    new_obj_info = update_obj_infomation.uptate_info.update_school_info(obj_table, obj_info.ID, rank_val, new_val, obj_info)
                    # 输出更新后的信息
                    print('》》》》更新完成，更新后的信息为》》》》')
    ...
    def editor_course_obj(obj_table):
        ...
        if obj_info == 'Fail':
            ...
            if update_attr in style.course_attr:
                ...
                try:
                    # 执行更新操作
                    new_course_info = update_obj_infomation.uptate_info.update_course_info(obj_table, obj_info.ID,
        ...
        if old_obj_info == 'Fail':
            ...
            if update_attr in style.teacher_arrt:
                ...
                try:
                    ...
                    new_teacher_info = update_obj_infomation.uptate_info.update_teacher_info(old_obj_info,
                                                                                             old_obj_info.ID, rank_name,
                                                                                             new_val, obj_table)
                    # 输出更新后的信息
                    time.sleep(1)
    ...
    def editor_classes_info(obj_table):
        print('\033[30;1m您现在操作的是修改%s信息\033[0m' % style.menu_dict[obj_table])
        edit_obj_id = input('\033[31;1m请输入您需要修改的%s的ID>>：\033[0m' % style.menu_dict[obj_table])
        # 查询此ID是否存在
        old_obj_info = db_operator.operator_db.search_id_in_db(edit_obj_id, obj_table)
        ...
        if old_obj_info == 'Fail':
            ...
            if update_attr in style.classes_arrt:
                ...
                try:
                    ...
                    print('\033[30;1m更新中，请稍侯！！！\033[0m')
                    new_classes_info = update_obj_infomation.uptate_info.updata_classes_info(obj_table, old_obj_info.ID, rank_name, new_val, old_obj_info)
                    # 输出更新后的信息
                    time.sleep(1)
                    print('\033[30;1m》》》》更新完成，更新后的信息为》》》》\033[0m')
